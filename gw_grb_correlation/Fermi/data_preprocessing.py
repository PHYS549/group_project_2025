...
def create_dataframe_and_name_column_from_data_files(data_type, PRINT_HEAD=False):
    """
    Creates a DataFrame from .npy files and assigns column names based on data type.
    ...
    if data_type == 'time':
        ...
        detectors = [f"n{i}" for i in range(10)] + ["na", "nb", "b0", "b1"]
        df.columns = ['ID', 'TSTART', 'TSTOP', 'T90', 'DATE'] + \
                     [f"{detector}_PH_CNT" for detector in detectors] + \
                     ['RA', 'DEC'] + ['QSJ_1', 'QSJ_2', 'QSJ_3', 'QSJ_4'] + \
                     [f"{detector}_TRIG" for detector in detectors]
...
def merge_all_datatypes_in_fermi(time_df, tte_df, location_df, poshist_df, trigger_df, print_info=False):
    """
    Merges all available Fermi data types into a single DataFrame.
...
def download_and_preprocess_fermi_data(start_year, end_year, download_or_not=True):
    """
    Downloads (optional) and preprocesses all Fermi data types into a single DataFrame.
    ...
    np.save(output_dir + "fermi_data.npy", merged_data.to_records(index=False))
    print(f"\nPreprocessed data saved to {output_dir}")

    merged_data.to_csv(output_dir + "fermi_data.csv", index=False)
    print(f"\nPreprocessed data saved to {output_dir}")
